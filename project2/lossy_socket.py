...
class SimulationParams:
    def __init__(
        ...
        become_reliable_after: float = 100000.0,
    ):
    ...
    def sendto(self, message: bytes, dst: Tuple[str, int]):
        ...
        if random.random() < sim.loss_rate and not sim.forced_reliable():
            ...
            Timer(
                (
                    0
                    if sim.forced_reliable()
                    else random.random() * sim.max_delivery_delay
                ),
                lambda: super(self.__class__, self).sendto(message, dst),
            ).start()
